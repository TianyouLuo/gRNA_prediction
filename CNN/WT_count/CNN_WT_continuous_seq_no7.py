...
class DeepSeqCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        x_concat = torch.cat((x0, x1, x2), dim=1) # size: [:,260,10]
        x_concat = x_concat.view(-1, 220*10)
...
ckptPATH = resultdir + '/models/Maria_plasmid_full-MSE-seq-no7fil-fold'+str(fold)+'-'+args.date+'.pth'
...
PD.to_csv(resultdir + '/Maria_plasmid_full-MSE-seq-no7fil-fold'+str(fold)+'-'+args.date+'.csv')
