...
class DeepSeqCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        x_concat = torch.cat((x0, x2), dim=1) # size: [:,260,10]
        x_concat = x_concat.view(-1, 120*10)
...
ckptPATH = resultdir + '/models/Maria_plasmid_full-MSE-seq-no37fil-fold'+str(fold)+'-'+args.date+'.pth'
...
PD.to_csv(resultdir + '/Maria_plasmid_full-MSE-seq-no37fil-fold'+str(fold)+'-'+args.date+'.csv')
