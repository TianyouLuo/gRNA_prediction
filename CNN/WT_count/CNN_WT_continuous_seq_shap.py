...
parser.add_argument('--n_shap_ref', type=int, default=1000, help='how many training data to use as SHAP background')
...
rdm = np.random.choice(len(sequence), size = args.n_shap_ref, replace = False)

test_shap_seq = torch.tensor(test_sequence_onehot, dtype=torch.float32).to(device)
train_shap_seq = torch.tensor(sequence_onehot, dtype=torch.float32).to(device)

background = train_shap_seq[rdm]

e = shap.DeepExplainer(CNN, [background])
test_shap_values = e.shap_values([test_shap_seq])
train_shap_values = e.shap_values([train_shap_seq])
...
np.save(resultdir + '/shap/WT_continuous_seqshap-fold'+str(fold)+'-test-'+args.date+'.npy', test_shap_seq_val)
np.save(resultdir + '/shap/WT_continuous_seqshap-fold'+str(fold)+'-train-'+args.date+'.npy', train_shap_seq_val)

# test_seq_top = test_sequence_onehot[test['avgWTcount_K562'] >= 5000]
# test_seq_bottom = test_sequence_onehot[(test['avgWTcount_K562'] < 400) & (test['avgWTcount_K562'] > 125)]

# test_top_explanations = np.sum(test_shap_seq_val[test['avgWTcount_K562'] >= 5000], axis=1)[:,None,:] * test_seq_top
# test_bottom_explanations = np.sum(test_shap_seq_val[(test['avgWTcount_K562'] < 400) & (test['avgWTcount_K562'] > 125)], axis=1)[:,None,:] * test_seq_bottom

# from deeplift.visualization import viz_sequence
# for explanations in test_top_explanations:
#     viz_sequence.plot_weights(explanations, subticks_frequency = 5)