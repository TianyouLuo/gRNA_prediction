...
class DeepSeqCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        # x_concat = torch.cat((x0, x1), dim=1) # size: [:,260,10]
        # x_concat = x_concat.view(-1, 50*10)
        x_concat = x0.view(-1, 50*10)
...
ckptPATH = resultdir + '/models/Maria_plasmid_full-MSE-seq-no357fil-fold'+str(fold)+'-'+args.date+'.pth'
...
PD.to_csv(resultdir + '/Maria_plasmid_full-MSE-seq-no357fil-fold'+str(fold)+'-'+args.date+'.csv')
