...
hic <- read_tsv("/proj/yunligrp/projects/HiC/wtc11/WTC11.union-loops.bedpe",col_names = c("chr1","start1","end1","chr2","start2","end2","method"))
...
library(plyranges)
library(data.table)
dat <- vroom("/proj/milovelab/mu/dukeproj/data/mhc/ipsc.latentvar.MAST_expect_cells.grna_de.markers.aggr.txt.gz")
dat2 <- vroom("/proj/milovelab/mu/dukeproj/data/mhc/ipsc.expect_cells.grna.de.markers.MAST.annotatedfull.final.update20220117.LRB.tsv")
# dat <- vroom("/proj/milovelab/mu/dukeproj/data/mhc/k562.expect_cells.grna.de.markers.MAST.annotatedfull.final.update20220117.LRB.tsv")
...
H3K27ac_cpm1kb<-read.table("/proj/milovelab/mu/dukeproj/K562-supp-tables/wtc_h3k27ac_cpm1kb.csv",header = T)
H3K27ac_cpm1kb_gr <- makeGRangesFromDataFrame(H3K27ac_cpm1kb, keep.extra.columns=TRUE)

## read ATAC-seq data
ATAC_cpm1kb<-read.table("/proj/milovelab/mu/dukeproj/K562-supp-tables/wpc_ATAC_cpm1kb.csv",header = T)
ATAC_cpm1kb_gr <- makeGRangesFromDataFrame(ATAC_cpm1kb, keep.extra.columns=TRUE)

## read H3K4me3 data
H3K4me3_cpm1kb<-read.table("/proj/milovelab/mu/dukeproj/K562-supp-tables/wtc_h3K4me3_cpm1kb.csv",header = T)
...
dat$deltagb <-deltagb
write.table(dat,file="/proj/milovelab/mu/dukeproj/data/mhc/ipsc_onegene_full.csv",row.names = F)

dat <- read.table(file="/proj/milovelab/mu/dukeproj/data/mhc/ipsc/ipsc_onegene_full.csv", header = T)
dat %>%
  select(grna.chr, grna.start, grna.end, grna, dhs, grna.strand) %>%
  write_tsv("/proj/milovelab/mu/dukeproj/data/mhc/ipsc/ipsc_coordinates.tsv", col_names = F)

gene_ess <- read.table(file="/proj/milovelab/mu/dukeproj/data/mhc/ipsc/ipsc_OGEE.txt", header = T)

library(tidyverse)
dat_cmb <- dat |> left_join(gene_ess[,c("grna","OGEE_prop_Essential")], by="grna")
DHS <- dat_cmb$dhs  
...
OGEE = fread("/proj/milovelab/mu/dukeproj/data/mhc/ipsc/ipsc_OGEE.txt")
OGEE_downstream = fread("/proj/milovelab/mu/dukeproj/data/mhc/ipsc/ipsc_OGEE_downstream.txt")
dat = dat %>% 
...
write_tsv(dat, file.path(dir2, "ipsc_full.tsv"))
...
write_csv(dat_sel, file.path(dir2, "ipsc-rawp0.05.csv"))
dat_sel <- read_csv(file.path(dir2, "ipsc-rawp0.05.csv"))
nrow(dat_sel) # 9570
## randomly sample dhs to split train and test data
## Binary classfication
set.seed(2050)
dhs <-dat_sel$dhs %>% unique() %>% sort() #580
...
for (i in 1:5){
  ...
  write_csv(train, file.path(dir2, paste0("ipsc-pfdr0.05-pfdr0.2-binary-fold",i,"-train.csv")))
  write_csv(test, file.path(dir2, paste0("ipsc-pfdr0.05-pfdr0.2-binary-fold",i,"-test.csv")))
}
